...
void get_post_comments_list(MYSQL *conn, int post_id, int *comment_ids_out, int *comment_count_out);
void view_comment_detail_menu(MYSQL *conn, int comment_id, const char *logged_id);
  ...
  while (1) {
    ...
    if (choice == 0) {
      ...
      if (category_id == 2 && !is_user_club_leader(conn, logged_id)) {
        printf("❌ 글 작성 권한이 없습니다. (전공 동아리장만 작성할 수 있습니다.)\n");
      } else {
...
int print_post_detail(MYSQL *conn, int post_id) {
  char query[512];
  sprintf(query, "SELECT p.title, p.content, u.nickname, p.created_at "
    ...
    if (choice == 0) {
      ...
      if (comment_count > 0) {
        ...
        if (comment_num > 0 && comment_num <= comment_count) {
          view_comment_detail_menu(conn, comment_ids[comment_num - 1], logged_id);
...
void get_post_comments_list(MYSQL *conn, int post_id, int *comment_ids_out, int *comment_count_out) {
  char query[512];
  sprintf(query, "SELECT c.comment_id, u.nickname, c.content, c.created_at "
  ...
  while ((row = mysql_fetch_row(res))) {
    ...
    // 대댓글 목록 들여쓰기로 출력
    char sub_query[512];
    sprintf(sub_query, "SELECT u.nickname, c.content, c.created_at "
                       "FROM comments c JOIN users u ON c.user_idx = u.user_idx "
                       "WHERE c.parent_comment_id = %d "
                       "ORDER BY c.created_at ASC", c_id);
...
void view_comment_detail_menu(MYSQL *conn, int comment_id, const char *logged_id) {
  ...
  while (1) {
    char query[512];
    sprintf(query, "SELECT u.nickname, c.content, c.created_at "
                   "FROM comments c JOIN users u ON c.user_idx = u.user_idx "
    ...
    sprintf(sub_query, "SELECT u.nickname, c.content, c.created_at "
                       "FROM comments c JOIN users u ON c.user_idx = u.user_idx "
                       "WHERE c.parent_comment_id = %d "
                       "ORDER BY c.created_at ASC", comment_id);
    MYSQL_RES *sub_res = NULL;
    if (mysql_query(conn, sub_query) == 0) {
      sub_res = mysql_store_result(conn);
    }
    ...
    if (choice == 0) {
      ...
      sprintf(post_id_query, "SELECT post_id FROM comments WHERE comment_id = %d", comment_id);
      int post_id = 0;
      if (mysql_query(conn, post_id_query) == 0) {
        MYSQL_RES *p_res = mysql_store_result(conn);
        if (p_res) {
          MYSQL_ROW p_row = mysql_fetch_row(p_res);
          if (p_row) post_id = atoi(p_row[0]);
          mysql_free_result(p_res);
        }
      }
      
      if (post_id > 0 && insert_comment(conn, post_id, logged_id, sub_content, comment_id)) {
